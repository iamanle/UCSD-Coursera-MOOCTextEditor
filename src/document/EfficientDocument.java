...
public class EfficientDocument extends Document {
	...
	private void processText()
	{
		...
		for(int j = 0; j < tokens.size(); j++){
			String word = tokens.get(j).toLowerCase();
			boolean validWord = isWord(word);
			if(!validWord || j == tokens.size()-1) numSentences++;
			if(validWord){
				numWords++;
				int prevVowelPos = Integer.MIN_VALUE;
				int syllables = 0;
				for(int i = 0; i < word.length(); i++){
					if(vowels.indexOf(word.charAt(i)) >= 0){
						if(i == word.length() - 1 && word.charAt(i) == 'e' && syllables > 0)
							break;
						if(prevVowelPos != i-1){
							++syllables;
						}
