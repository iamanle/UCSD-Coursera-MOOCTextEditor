...
public class NearbyWords implements SpellingSuggest {
		...
		for(int i = 0; i <= s.length(); i++){
			for(int charCode = (int)'a'; charCode <= (int)'z'; charCode++){
				...
				if(!currentList.contains(candidate)
						&&(!wordsOnly || dict.isWord(candidate)) 
						&& !s.equals(candidate)){
	...
	public void deletions(String s, List<String> currentList, boolean wordsOnly ) {
		for(int i = 0; i < s.length(); i++){
			...
			if(!s.equals(candidate) &&
					!currentList.contains(candidate)&&
					(!wordsOnly || dict.isWord(candidate))){
				currentList.add(candidate);
			}
		}
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		int count = 0;
		int loopCount = 0;
		while(count < numSuggestions && !queue.isEmpty() && loopCount < THRESHOLD){
			String candidate = queue.remove(0);
			List<String> oneDistance = distanceOne(candidate, true);
			loopCount++;
			for(String w : oneDistance){
